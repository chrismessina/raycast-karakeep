...
- Commands (entrypoints)
  ...
  - `quickBookmark.tsx` — quick bookmark the current browser tab (no-view mode). Automatically captures the active browser tab URL and creates a bookmark without opening a view.
...
- Hooks
  ...
  - `useFrecencySorting` — from `@raycast/utils`, persists visit signals for better default ordering in `bookmarks.tsx`.
