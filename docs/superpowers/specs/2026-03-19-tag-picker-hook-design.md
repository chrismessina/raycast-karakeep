...
1. Extract all tag-picking state and logic into a single reusable `useTagPicker` hook.
2. Add list and tag fields to `BookmarkEdit`.
...
- Changes to tag-picking UI behavior (no new UX)
- AI tag suggestion feature (separate TODO item)
- Any changes to the create forms beyond the hook refactor

---

## List Pre-population Limitation (Global)

Both `BookmarkEdit` and `NoteEdit` list dropdowns default to empty on open. The bookmark object as returned by the API does not include current list membership, and a separate fetch is out of scope. This is a known limitation for both edit forms.
...
interface UseTagPickerReturn {
  ...
  addedTagIds: string[];          // selectedTagIds minus initialTagIds — use for attach-only edit logic
...
- `addedTagIds` — derived: `selectedTagIds.filter(id => !initialTagIds?.includes(id))`. Used by edit forms to attach only newly added tags without re-attaching existing ones.
- `buildTagsToAttach` — calls `buildTagsToAttach(selectedTagIds, newTagItems)` from `src/utils/tags.ts`, closing over internal state
- `reset` — resets `selectedTagIds` to `[]`, `newTagItems` to `[]`, `pendingInput` to `""`, ref to `[]`. **Only call from create forms.**

Constants `NEW_TAG_PREFIX` and `TAG_PICKER_NOOP_VALUE` live in this file (not exported to consumers; only `src/utils/tags.ts` needs `NEW_TAG_PREFIX` and imports it from here).

**`initialTagIds` behavior:** Seeds `selectedTagIds` on mount. If the user deselects a pre-existing tag, it is removed from `selectedTagIds` — no detach API is called. The hook does not distinguish pre-existing from newly added tags at the state level; use `addedTagIds` in submit logic to attach only the delta.
...
The hook's no-arg `buildTagsToAttach()` delegates to this function, passing its own state. This file is not called directly by form components — they call the hook's method.
...
- List (Dropdown) — defaults to empty (see global limitation above)
...
**Submit:**
1. `fetchUpdateBookmark(bookmark.id, { title: values.title.trim(), text: values.content.trim() })` — `text` is the API field name for note body content on the PATCH endpoint, matching `bookmark.content.text` on the read side.
2. If `addedTagIds.length > 0`: `fetchAttachTagsToBookmark(bookmark.id, buildTagsToAttach())` — uses `addedTagIds` filtering so only newly added tags are sent, not pre-existing ones.
3. If a list is selected: `fetchAddBookmarkToList(values.list, bookmark.id)`
...
- Add: `const { selectedTagIds, newTagItems, pendingInput, onTagIdsChange, onPendingInputChange, commitPendingTag, buildTagsToAttach, reset } = useTagPicker({ tags })`
- In `onSubmit`: replace inline loop with `buildTagsToAttach()`. Call `reset()` is not needed here — `createBookmark` uses `pop()` on success, not `setContent("")`.
...
Add:
- `useGetAllLists` → List Dropdown (defaults to empty)
...
- If `values.list`: `fetchAddBookmarkToList(values.list, bookmark.id)`

No list-removal API call (non-goal). List dropdown defaults to empty (see global limitation).
...
`asset` bookmarks fall through to `BookmarkEdit` via the `else` branch — this is intentional.
...
Form components
  ...
  └── BookmarkEdit, NoteEdit: useTagPicker({ tags, initialTagIds })
      └── onSubmit: addedTagIds.length > 0 → fetchAttachTagsToBookmark(buildTagsToAttach())
      └── no reset() call
```
...
- Detaching existing tags on edit — deselecting a pre-existing tag does not call a detach API. Tag removal via picker is a follow-up.
- Pre-populating list on edit forms — list dropdown defaults to empty for both `BookmarkEdit` and `NoteEdit`. Requires API support or a separate fetch.
- Removing a bookmark from a list on edit — no list-removal API call is made on submit.
