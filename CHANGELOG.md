...
- Rewrote the settings labels for Raycast's current preferences UI, where the large text comes from the label rather than the title
...
- The list icon field is now a searchable picker instead of a text field that asked you to type `:` for an emoji picker that doesn't exist in Raycast
- Fixed every command with a list dropdown firing one request per list on open. A bookmark count was fetched for each list, which cost 45 requests on a 45-list library and reported the wrong number anyway — it was capped at the page size, so any list with more than 10 bookmarks showed "(10)". The count is gone; the requests with it
