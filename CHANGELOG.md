...
- Copy URL is available again on a bookmark. It shared a condition with Open URL — which is hidden when it is already the ↵ action — so anyone using the default "Open in Browser" setting never saw Copy at all
